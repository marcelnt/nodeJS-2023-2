...
//Retorna todos os estados apenas a sigla
...
//Retorna os dados de um estado mais a sua capital tendo como base a sigla (Sigla, nome e capital)
...
//Retorna os dados de um estado tendo como base a Regiao (UF e Descricao)
const getEstadosRegiao = function(regiaoEstados){
    ...
    if(typeof(regiao) != 'undefined')
    {
        ...
        if (regiao != '' )
        {
            listaDeEstados.estados.forEach (item => {
                //Localiza um item no array (indexOf())
               let nome = item.regiao.toUpperCase();

                if(nome == regiao)
                {
                    //Criamos as chaves uf e descricao para enviar pelo JSON
                    estado.push({uf:item.sigla, descricao:item.nome});
                    erro = false;
                }
            });

            estadoJSON.regiao = regiao;
            estadoJSON.estados = estado;
        }
    }
...
//Retorna os dados da capital do Brasil (sigla, descricao, capital_estado, regiao, capital_pais_ano_inicio, capital_pais_ano_termino )
const getCapitalPais = function(){
   
    //Cria um objeto do tipo JSON
    let estado = [];
    let estadosCapitaisJSON = {};
    ...
    let erro = true;
        ...
        estadosCapitaisJSON.capitais = estado;


    if(erro)
        return false;
    else    
        return estadosCapitaisJSON;
        ...
        if (sigla != ''  && sigla.length == 2 )
        {
          ...
          listaDeEstados.estados.forEach(item => {
             ...
             if(item.sigla.indexOf(sigla.toUpperCase()) == 0)
             {
                ...
                listaCidadesJSON.uf = item.sigla
                listaCidadesJSON.descricao = item.nome
                listaCidadesJSON.quantidade_cidades = listCidades.length
                listaCidadesJSON.cidades = listCidades
             }
          });
